...
def load_confidence_intervals(input_dir: str):
    ...
    for metric_dir in glob(f'{input_dir}/*'):
        ...
        for coef in COEFS:
            samples = list(map(float, open(f'{metric_dir}/{coef}.txt', 'r').read().splitlines()))
...
def main(args):
    ...
    for coef in COEFS:
        ...
        for i, metric in enumerate(reversed(metrics)):
            ...
            fab_judged_width = max(fab_judged[metric][coef]) - min(fab_judged[metric][coef])
            fab_all_width = max(fab_all[metric][coef]) - min(fab_all[metric][coef])
            bha_judged_width = max(bha_judged[metric][coef]) - min(bha_judged[metric][coef])
